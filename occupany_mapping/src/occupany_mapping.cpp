...
{
   mapParams.width = 1200;
   mapParams.height = 1200;
...
void OccupanyMapping(std::vector<GeneralLaserScan>& scans,std::vector<Eigen::Vector3d>& robot_poses)
{
    ...
    for(int i = 0; i < scans.size();i++)
    {
        ...
        for(int id = 0; id < scan.range_readings.size();id++)
        {
            ...
            GridIndex hitIndex = ConvertWorld2GridIndex(world_x,world_y);
            if(isValidGridIndex(hitIndex) != true || (hitIndex.x == robotIndex.x && hitIndex.y == robotIndex.y))continue;


            std::vector<GridIndex> freeGrid = TraceLine(robotIndex.x, robotIndex.y, hitIndex.x, hitIndex.y);
            for(int grid = 0; grid < freeGrid.size(); grid++)
            {
                int line_free_grid = GridIndexToLinearIndex(freeGrid[grid]);
                pMap[line_free_grid] += mapParams.log_free;
                if(pMap[line_free_grid] < mapParams.log_min)
                {
                    pMap[line_free_grid] = mapParams.log_min;
                }
            }

            int line_hit_grid = GridIndexToLinearIndex(hitIndex);
            pMap[line_hit_grid] += mapParams.log_occ;
            if(pMap[line_hit_grid] > mapParams.log_max)
            {
                pMap[line_hit_grid] = mapParams.log_max;
            }
...
int main(int argc, char** argv)
{
    ...
    std::string basePath = "/home/fengyusu/catkin_ws/src/lidar_slam/occupany_mapping/data";
